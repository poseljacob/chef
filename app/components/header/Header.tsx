...
export function Header({ hideSidebarIcon = false }: { hideSidebarIcon?: boolean }) {
  ...
  const isGetBotsExternal = rootLoaderData?.ENV?.GETBOTS_EXTERNAL_MODE === '1';
        ...
        <a href="/">
          {isGetBotsExternal ? (
            <div className="flex items-center gap-3">
              <span className="inline-flex size-8 items-center justify-center rounded-lg bg-[#2563eb] text-sm font-bold text-white">
                BT
              </span>
              <div className="leading-tight">
                <div className="text-[1.2rem] font-semibold text-content-primary">GetBots Studio</div>
                <div className="text-xs text-content-secondary">Build apps and sync to your bots</div>
              </div>
            </div>
          ) : (
            <>
        ...
        <ClientOnly>
          {() => (
            <div className="ml-auto flex items-center gap-2">
              ...
              {chat.started && (
                <>
                  {!isGetBotsExternal && <PromptDebugButton />}
                  {!isGetBotsExternal && isPaidPlan === false && referralStats && referralStats.left > 0 && (
                    <ReferButton />
                  )}
                  <DownloadButton />
                  <ShareButton />
